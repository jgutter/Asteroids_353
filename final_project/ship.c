...
void draw_ship(Ship * ship) {
	uint8_t ship_indx = 0;
	int ship_color = SHIP_COLOR;
	if (ship->invincibility > 0)
		ship_color = INVINCIBILE_SHIP;
	//need a uniform bitmap array to access correct ship image via simple calculation of angle
	ship_indx = abs((ship->angle - 90)) / 10;
	if ((ship->angle < 270) && (ship->angle > 90)) {
		ship_indx = BITMAP_REF - ship_indx;
	}
