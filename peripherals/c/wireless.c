...
{
	reg = (reg | NRF24L01_CMD_R_REGISTER); // to ensure a read command
	uint8_t received[2] = {0, 0};
	uint8_t transmit[2] = {reg, reg};	 //transmit[1] is a don't care
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, transmit, 2, received);
	wireless_CSN_high();
	return received[1];
}
...
{
	reg = (reg | NRF24L01_CMD_W_REGISTER); // to ensure reg is a write command
	uint8_t received[2] = {0, 0}; //don't cares
	uint8_t transmit[2] = {reg, data};
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, transmit, 2, received);
	wireless_CSN_high();
}
...
{
	uint8_t transmit_cmd = (NRF24L01_TX_ADDR_R | NRF24L01_CMD_W_REGISTER);
	uint8_t receive[5];
	wireless_CSN_low();
	spiTx(RF_SPI_BASE,&transmit_cmd, 1, receive);
	spiTx(RF_SPI_BASE,	tx_addr, 5, receive);
	wireless_CSN_high();
...
{
	uint8_t transmit_cmd = NRF24L01_CMD_W_TX_PAYLOAD;
	uint8_t receive[5];
	wireless_CSN_low();
	spiTx(RF_SPI_BASE,&transmit_cmd, 1, receive);
	spiTx(RF_SPI_BASE,(uint8_t *)&data + 3, 1, receive);
	spiTx(RF_SPI_BASE,(uint8_t *)&data + 2, 1, receive);
	spiTx(RF_SPI_BASE,(uint8_t *)&data + 1, 1, receive);
	spiTx(RF_SPI_BASE,(uint8_t *)&data, 1, receive);
	wireless_CSN_high();
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	*data = 0;
	...
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, tx_data, 1, &status);
	spiTx(RF_SPI_BASE, tx_data, 1, (uint8_t *)(data + 3));
	spiTx(RF_SPI_BASE, tx_data, 1, (uint8_t*)(data + 2));
	spiTx(RF_SPI_BASE, tx_data, 1, (uint8_t*)(data + 1));
